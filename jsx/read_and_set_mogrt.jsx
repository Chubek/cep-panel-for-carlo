...
var readFileAndParse = function readFileAndParse(seqName) {
  ...
  testtextfile.close();
  var jsons = [];
  var spltStrJsons = fileContentsString.split("\n\t");

  for (i = 0; i < spltStrJsons.length; i++) {
    if (spltStrJsons[i].length > 1) {
      jsons.push(JSON.parse(spltStrJsons[i]));
    }
  }
...
for (o = 0; o < app.project.sequences.length; o++) {
  var seqName = app.project.sequences[o].name;
  var jsons = readFileAndParse(seqName);
  ...
  for (m = 0; m < jsons.length; m++) {
    ...
    var k = jsons[m]["k"];
    var n = jsons[m]["n"];
    var value = JSON.stringify(jsons[m]["mogrtJson"]);
    $.writeln("Indices -> i: ".concat(_i, ", j: ").concat(j, ", k: ").concat(k, ", n: ").concat(n));

    app.project.sequences[o].videoTracks[_i].clips[j].components[k].properties[n].setValue(value, 1);
  }
}
