...
var getMogrtText = function getMogrtText(sequence) {
  ...
  for (i = 0; i < videoTracks.length; i++) {
    $.writeln("i header is pointed to ".concat(i));
    var clips = videoTracks[i].clips;

    for (j = 0; j < clips.length; j++) {
      $.writeln("j header is pointed to ".concat(j));
      var components = clips[j].components;

      for (k = 0; k < components.length; k++) {
        $.writeln("k header is pointed to ".concat(k));
        var properties = components[k].properties;

        for (n = 0; n < properties.length; n++) {
          $.writeln("n header is pointed to ".concat(n));

          try {
            if (properties[n].getValue().length < 10) {
              $.writeln("Smaller than 10, continuing...");
              continue;
            }
            ...
            jsonObject = {
              ...
              "k": k,
              "n": n,
              "mogrtJson": JSON.parse(properties[n].getValue())
            };
            $.writeln("Got json");
            jsons.push(jsonObject);
          } catch (_unused) {
            $.writeln("Error getting JSON.");
            continue;
          }
        }
      }
...
for (i = 0; i < sequences.length; i++) {
  ...
  var jsons = getMogrtText(sequences[i]);
  var jsonsStr = "";
